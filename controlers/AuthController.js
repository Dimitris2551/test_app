...
class AuthController {
    ...
    getSecret(req, res){
        ...
        console.log(`req.secret: ${req.secrets.docs}`);
        if(req.auth)
        {
            res.json(req.secrets);
        }
        else
        {
            res.json({auth: req.auth, message: "Can't really show you the secrets. you need to be logged in for that"});
        }
    }
}
